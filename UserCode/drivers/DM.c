...
void DM_Init(DM_t* hdm, const DM_Config_t* dm_config)
{
    static uint8_t initdata[8] = {
        0xFF, 0XFF, 0XFF, 0xFF, 0XFF, 0XFF, 0XFF, 0XFC}; // DM电机初始化需要发送的数据
    ...
    hdm->motor_type = dm_config->motor_type;
    /* 注册回调 */
...
void DM_MIT_SendSetCmd(DM_t* hdm, float p_des, float v_des, float kp, float kd, float t_ff)
{
    ...
    data[1] = (p & 0xFF);
    data[2] = (v >> 4);
    data[3] = ((v & 0xF) << 4) | (kp_int >> 8);
    data[4] = (kp_int & 0xFF);
    data[5] = (kd_int >> 4);
    data[6] = ((kd_int & 0xF) << 4) | (t >> 8);
    data[7] = (t & 0xFF);

    CAN_SendMessage(hdm->hcan,
                    &(CAN_TxHeaderTypeDef){
                        .StdId = DM_MODE_MIT | hdm->id0, // 0x000 + ID
                        .IDE   = CAN_ID_STD,
                        .RTR   = CAN_RTR_DATA,
                        .DLC   = 8,
                    },
